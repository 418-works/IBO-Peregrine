...
public class SongbirdProofer {
    ...
    private final String[] reqCategories = { "original order", "who entered", "master page #", "location", "bandsize",
            "disposition", "band #", "species", "age", "ha", "ha2", "sex", "hs", "hs2", "skull", "bp", "cp",
            "fat", "muscle", "b mlt", "ff mlt", "ff wear", "wing", "tail", "weight", "status", "date", "yyyy", "mm",
            "dd", "julian", "cap time", "site", "net #", "parasites?", "disp", "notes", "proofing notes" };
    ...
    public static void main(String[] args) {
        new SongbirdProofer(new File("bad.csv"));
    }
     ...
     * @param path
     */
    public SongbirdProofer(File file) {
        try {
            Scanner csvScanner = new Scanner(file);
            PrintWriter writer = new PrintWriter("proofed.csv");
            String categories = csvScanner.nextLine();
            writer.println(categories);
            String[] fileCategories = categories.split(",");
    ...
    public void proofSpecies(DataLine line) {
        ...
        if (!binarySearch(validSpecies, line.getSpecies().toLowerCase(), 0, validSpecies.length - 1)) {
            line.setCustomNotes(  "     SPECIES***:" + line.getSpecies() + " is rare or does not exist.");
        }
    }
    ...
    public void proofAge(DataLine line) {
        ...
        if (!(age == 0 || age == 1 || age == 2 || age == 4 || age == 5 || age == 6)) {
            line.setCustomNotes(  "     AGE***: Invalid age. Age must be 0 1 2 4 5 or 6");
        }
    }
    ...
    public void proofSex(DataLine line) {
        ...
        if (!(sex.equals("M") || sex.equals("F") || sex.equals("U"))) {
            line.setCustomNotes(  "     SEX***: Invalid sex. Sex must be M F or U");
        }
    }
    ...
    public void proofAgeBPCP(DataLine line) {
        ...
        if (age == 0 || age == 2 || age == 4) {
            if (bp > 0 || cp > 0) {
                line.setCustomNotes(  "     AGE BP/CP MISMATCH***: BP and CP must both be 0 if age = 0 2 or 4");
            }
        }
    }
    ...
    public void proofHowSexed(DataLine line) {
        ...
        if (sex.equals("F")) {
            if (!(hs.equals("PL") || hs.equals("BP") || hs.equals("WL"))) {
                line.setCustomNotes(
                    "     HS/SEX MISMATCH***: Invalid how sexed value. Acceptable hs values for females are PL BP and WL");
            } else if (hs.equals("BP") && (bp == 0 || bp == -1)) {
                line.setCustomNotes(  "     HS***: If sexed by BP BP value cannot be blank or 0");
            }
        } else if (sex.equals("M")) {
            // ask about CP
            if (!(hs.equals("PL") || hs.equals("CL") || hs.equals("WL"))) {
                line.setCustomNotes(
                    "     HS/SEX MISMATCH***: Invalid how sexed value. Acceptable hs values for males are PL CL and WL");
            } else if (hs.equals("CL")) {
                if (!(cp == 2 || cp == 3)) {
                    line.setCustomNotes(  "     HS***: If sexed by CL CP value must be 2 or 3");
                }
            }
        } else if (sex.equals("U")) {
            if (!(hs.equals("") || hs.equals("IC"))) {
                line.setCustomNotes(
                    "     HS/SEX MISMATCH***: Invalid how sexed value. Acceptable hs values for sex = U are blank and IC");
            }
        }
    ...
    public void proofFFMolt(DataLine line) {
        ...
        if (!(ffMlt.equals("N") || ffMlt.equals("S") || ffMlt.equals("J") || ffMlt.equals("A") || ffMlt.equals(""))) {
            line.setCustomNotes(
                "     FFMLT***: Invalid FF molt value. Acceptable values are N S J A and blank");
        }
    }
    ...
    public void proofBP(DataLine line) {
        ...
        String sex = line.getSex();
        if (bp > 5) {
            line.setCustomNotes(  "     BP***: BP cannot exceed 5");
        }
        if (bp != 0 && sex.equals("M")) {
            line.setCustomNotes("       BP/SEX MISMATCH***: if sex is M BP must be 0");
        }
    }
    ...
    public void proofCP(DataLine line) {
        ...
        String sex = line.getSex();
        if (cp > 3) {
            line.setCustomNotes(  "     CP***: CP cannot exceed 3");
        }
        if (cp != 0 && sex.equals("F")) {
            line.setCustomNotes("       CP/SEX MISMATCH***: if sex is F CP must be 0");
        }
    }
    ...
    public void proofFat(DataLine line) {
        int fat = line.getFat();
        if (fat > 5) {
            line.setCustomNotes(  "     FAT***: Fat cannot exceed 5");
        }
    }
    ...
    public void proofBMlt(DataLine line) {
        ...
        if (bMlt > 4) {
            line.setCustomNotes(  "     BMLT***: Body molt cannot exceed 4");
        }
    }
    ...
    public void proofFFWear(DataLine line) {
        ...
        if (ffWear > 5) {
            line.setCustomNotes(  "     FFWEAR***: FF wear cannot exceed 5");
        }
    }
    ...
    public void proofMuscle(DataLine line) {
        ...
        if (!(muscle == -1 || muscle == 2.5 || muscle == 3 || muscle == 4 || muscle == 5)) {
            if (muscle == 1 || muscle == 2) {
                if (line.getNotes().equals("")) {
                    line.setCustomNotes("       MUSCLE***: Muscle cannot be 1 or 2 without a note");
                }
            } else {
                line.setCustomNotes(
                    "     MUSCLE***: Muscle value invalid. Value must be 2.5 3 4 or 5. 1 and 2 with notes is acceptable");
            }
        }
    }
    ...
    public void proofSkullAge(DataLine line) {
        ...
        if (skull == 0 && !(age == 2 || age == 4)) {
            line.setCustomNotes(
                "     AGE/SK MISMATCH***: Age cannot be " + age + " when skull is 0. Age must be 2 or 4");
        } else if (skull == 1 && !(age == 2 || age == 4)) {
            line.setCustomNotes(
                "     AGE/SK MISMATCH***: Age cannot be " + age + " when skull is 1. Age must be 2 or 4");
        } else if (age != 2 && (skull == 3 || skull == 4)) {
            line.setCustomNotes(
                "     AGE/SK MISMATCH***: Age cannot be " + age + " when skull is 3 or 4. Age must be 2");
            ...
            if (age == 1 || age == 5 || age == 6) {
                ...
                if (!contains) {
                    line.setCustomNotes(  "     AGE/SK MISMATCH***: Age cannot be " + age
                        + " when skull is 5 and species is not an exception. Age must be 2");
                }
            } else {
                line.setCustomNotes(
                    "     AGE/SK MISMATCH***: Age cannot be " + age + " when skull is 5. Age must be 2");
            }
            ...
            line.setCustomNotes(  "     HA***: How Aged cannot be skull when skull = 7 8");
        }
    }
    ...
    public void proofLocation(DataLine line) {
        ...
        if (location.equals("")) {
            line.setCustomNotes(  "     LOCATION***: Location cannot be empty");
        }
    }
        ...
        if (!(bandSize.equals("") || bandSize.equals("0A") || bandSize.equals("0") || bandSize.equals("1")
            ...
            if (bandSize.equals("R")) {
                if (!disposition.equals("R")) {
                    line.setCustomNotes(  "     BAND SIZE***: Band size cannot be R when band code is not R");
                }
            } else if (bandSize.equals("U")) {
                if (!disposition.equals("U")) {
                    line.setCustomNotes(  "     BAND SIZE***: Band size cannot be U when band code is not U");
                }
            } else {
                line.setCustomNotes(
                    "     BAND SIZE***: Band size invalid. Acceptable values: 0A 0 1 1B 1A 2 3 3A 3B");
            }
        }
    }
    ...
    public void proofAgeFFMolt(DataLine line) {
        ...
        && !(ffMlt.equals("N") || ffMlt.equals("S") || ffMlt.equals("A") || ffMlt.equals(""))) {
            line.setCustomNotes(  "     AGE/FFMLT MISMATCH***: Valid FFMlt values for ages 1 5 6 are N S A");
            ...
            if (!(species.equals("ybch") || species.equals("spto") || species.equals("sosp") || species.equals("hofi")
                ...
                line.setCustomNotes(
                    "     AGE/FFMLT MISMATCH***: Valid FFMlt values for age 2 are N A if not an exception species");
            }
        } else if (age == 4 && !(ffMlt.equals("J") || ffMlt.equals(""))) {
            line.setCustomNotes(  "     AGE/FFMLT MISMATCH***: If age = 4 then FFMlt must be J");
        }
    }
    ...
    public void proofHowAgedFFMolt(DataLine line) {
        ...
        if (ha.equals("mr") && !(ffMlt.equals("S") || ffMlt.equals("J"))) {
            line.setCustomNotes(  "     HA/FFMLT MISMATCH***: If How Aged is MR then FFMolt cannot be blank");
        }
    }
    ...
    public void proofHowAgedFFWear(DataLine line) {
        ...
        if (ha.equals("ff") && ffWear < 0) {
            line.setCustomNotes(  "     HA/FFWEAR MISMATCH***: If How Aged is FF then FFWear cannot be blank");
        }
    }
    ...
    public void proofAgeFFWear(DataLine line) {
        ...
        if ((age == 5 || age == 6) && (ffWear == 0 || ffWear == 1)) {
            line.setCustomNotes(
                "     AGE/FFWEAR MISMATCH***: FF wear of 0 or 1 is suspicious for a bird of age 5 or 6.");
        } else if (age == 4 && ffWear >= 2) {
            line.setCustomNotes(  "     AGE/FFWEAR MISMATCH***: FF wear >= 2 is suspicious for a bird of age 4");
        } else if (age == 2 && ffWear >= 4) {
            line.setCustomNotes(  "     AGE/FFWEAR MISMATCH***: FF wear >= 4 is suspicious for a bird of age 2");
        }
    }
    ...
    public void proofWing(DataLine line) {
        ...
        if (wing > -1) {
            if (wing < 30 || wing > 200) {
                line.setCustomNotes(  "     WING***: Wing below 30 or above 200 is suspicious");
            }
        }
    }
    ...
    public void proofTail(DataLine line) {
        ...
        if (tail > -1) {
            if (tail < 30 || tail > 200) {
                line.setCustomNotes(  "     TAIL***: Tail below 30 or above 200 is suspicious");
            }
        }
    }
        ...
        if (weight > -1) {
            if (weight < 5) {
                if (!(species.equals("rcki") || species.equals("bchu") || species.equals("ruhu")
                    || species.equals("cahu"))) {
                    line.setCustomNotes(  "     WEIGHT***: " + weight
    ...
    public void proofDisposition(DataLine line) {
        ...
        String species = line.getSpecies().toLowerCase();
        if (disp.equals("")) {
            line.setCustomNotes(  "     BAND CODE***: Band code cannot be empty");
        } else if (disp.equals("8") || disp.equals("4")) {
            if (!(species.equals("bade") || species.equals("balo"))) {
                line.setCustomNotes(
                    "     BAND CODE***: Band codes 4 and 8 are only valid if species is BADE or BALO");
            }
        } else if (!(disp.equals("1") || disp.equals("5") || disp.equals("R") || disp.equals("U"))) {
            line.setCustomNotes(
                "     BAND CODE***: Invalid band code. Acceptable values: 1 5 R U or 4 and 8 if species = BADE BALO");
        } else if (species.equals("bade") || species.equals("balo")) {
            if (!(disp.equals("8") || disp.equals("4"))) {
                line.setCustomNotes(  "     BAND CODE: ***If band was destroyed or lost disp must be 4 or 8");
            }
    ...
    public void proofStatus(DataLine line) {
        ...
        if (!(status == 300 || status == 500 || status == 0)) {
            if (status == -1) {
                line.setCustomNotes(  "     STATUS***: Status field cannot be blank");
            } else {
                line.setCustomNotes(
                    "     STATUS***: " + status + " is an invalid status must be 300 or 500");
            }
        } else if (status == 500 && notes.equals("") && disp.equals("")) {
            line.setCustomNotes(  "     STATUS***: Notes and disp cannot be blank when status is 500");
        }
    }
        ...
        || disp.equals("P") || disp.equals("")) {
            if (!disp.equals("")) {
                if (status != 500 && !disp.equals("D")) {
                    line.setCustomNotes(  "     DISP***: If disp field is not empty status must be 500");
                }
                if (notes.equals("")) {
                    line.setCustomNotes(  "     DISP***: If disp field is not empty notes must be filled");
                }
            }
        } else {
            line.setCustomNotes(  "     DISP***: " + disp + " is an invalid disp value");
        }
    }

    public void proofDate(DataLine line) {
        String[] date = line.getDate().split("/"); 
        String species = line.getSpecies();
        int currentYear = Calendar.getInstance().get(Calendar.YEAR);
        try {
            int year = Integer.parseInt(date[2]);
            if ((year < 1997 || year > currentYear) && !(species.equals("bade") || species.equals("balo"))) {
                line.setCustomNotes("     DATE/YEAR***: Year cannot be before 1997 or after " + currentYear);
            }
            int month = Integer.parseInt(date[0]);
            if ((month < 2 || month > 11) && !(species.equals("bade") || species.equals("balo"))) {
                line.setCustomNotes( "     DATE/MONTH***: Month cannot be December or January");
            }
            int day = Integer.parseInt(date[1]);
            if ((day < 1 || day > 31) && !(species.equals("bade") || species.equals("balo"))) {
                line.setCustomNotes("     DATE/DAY***: Day cannot be < 1 or > 31");
            }
        }
        catch(NumberFormatException e) {
            line.setCustomNotes("       DATE***: date is invalid");
        }
    }
    ...
    public void proofYear(DataLine line) {
        ...
        if (year == -1 && !(species.equals("bade") || species.equals("balo"))) {
            line.setCustomNotes(  "     YYYY***: Year cannot be blank if band wasn't lost or destroyed");
        } 
        if ((year < 1997 || year > currentYear) && !(species.equals("bade") || species.equals("balo"))) {
            line.setCustomNotes(  "     DATE/YEAR***: Year cannot be before 1997 or after " + currentYear);
        }
    ...
    public void proofMonth(DataLine line) {
        ...
        if (month == -1 && !(species.equals("bade") || species.equals("balo"))) {
            line.setCustomNotes(  "     MM***: Month cannot be blank if band wasn't lost or destroyed");
        } else if ((month < 2 || month > 11) && !(species.equals("bade") || species.equals("balo"))) {
            line.setCustomNotes(  "     MM***: Month cannot be December or January");
        }
    }
    ...
    public void proofDay(DataLine line) {
        ...
        if (day == -1 && !(species.equals("bade") || species.equals("balo"))) {
            line.setCustomNotes(  "     DD***: Day cannot be blank if band wasn't lost or destroyed");
        } else if ((day < 1 || day > 31) && !(species.equals("bade") || species.equals("balo"))) {
            line.setCustomNotes(  "     DD***: Day cannot be < 1 or > 31");
        }
    }
    ...
    public void proofCapTime(DataLine line) {
        ...
        if (capTime < 650 || capTime > 1300) {
            line.setCustomNotes(  "     CAP TIME***: Cap time must be between 650 and 1300");
        } else if (capTime % 10 != 0) {
            line.setCustomNotes(  "     CAP TIME***: Cap time must end in 0");
        }
    }
    ...
    public void proofNet(DataLine line) {
        ...
        if (net != -1) {
            if (net < 1 || net > 10) {
                line.setCustomNotes(  "     NET***: Net # must be between 1 and 10");
            }
        }
    }
    ...
    public void proofNotes(DataLine line) {
        ...
        || notes.contains(" fly ")) {
            if (line.hasParasites().equals("Y")) {
                line.setCustomNotes(  "     NOTES/PARASITES***: Read notes and change parasites to Y if warranted");
            }
        }
    }
    ...
    public void proofHowAged(DataLine line) {
        ...
        if ((ha.equals("ey") || ha2.equals("ey")) && !(species.equals("spto") || species.equals("dowo")
            ...
            line.setCustomNotes(
                "     HA/SPECIES MISMATCH***: Check in Pyle to confirm that this species can be aged by eye color");
        }
    }
    ...
    public void proofAgeHAHS(DataLine line) {
        ...
        if (age == 0) {
            if (!(ha.equals("") || ha.equals("IC"))) {
                line.setCustomNotes(  "     HA***: invalid how aged for age 0");

            }
            if (!(hs.equals("") || hs.equals("IC"))) {
                if (!(species.equals("rcki") || species.equals("gcki"))) {
                    line.setCustomNotes(
                        "     HS***: known sex for a bird of unknown age is unlikely. exceptions = rcki gcki");
                }
            }
        } else if (age == 1 || age == 6) {
            ...
            if (!ha.equals("NL") && age == 6) {
                line.setCustomNotes(  "     HA/AGE MISMATCH***: how aged must be NL if age = 6");
                ...
                line.setCustomNotes(
                    "     HA/AGE MISMATCH***: how aged must be PL NL EY FF MB PC MR SK or TS if age = 1");
            }
            // PLBPCLWL blank
            if (!(hs.equals("PL") || hs.equals("BP") || hs.equals("CL") || hs.equals("WL") || hs.equals(""))) {
                line.setCustomNotes(
                    "     HS/AGE MISMATCH***: how sexed must be PL BP CL WL or blank if age = 1 or 6");
            }
        } else if (age == 5) {
            // "LP" MUST be in the first "how aged" field for all age 5's.
            // 2nd "how aged" field may include: PL EY FF MB PC MR SK TS or blank
            if (!ha.equals("LP")) {
                line.setCustomNotes(  "     HA/AGE MISMATCH***: how aged must be LP in the first column if age = 5");
            }
            if (!(ha2.equals("PL") || ha2.equals("EY") || ha2.equals("FF") || ha2.equals("MB") || ha2.equals("PC")
                ...
                || ha2.equals(""))) {
                line.setCustomNotes(
                    "     HA2/AGE MISMATCH***: how aged must be PL EY FF MB PC MR SK TS or blank if age = 5");
            }
            // PLBPCLWL blank
            if (!(hs.equals("PL") || hs.equals("BP") || hs.equals("CL") || hs.equals("WL") || hs.equals(""))) {
                line.setCustomNotes(  "     HS/AGE MISMATCH***: how sexed must be PL BP CL WL or blank if age = 5");
            }
        } else if (age == 2) {
            ...
            if (!(ha.equals("PL") || ha.equals("EY") || ha.equals("FF") || ha.equals("MB") || ha.equals("PC")
                ...
                line.setCustomNotes(
                    "     HA/AGE MISMATCH***: how aged must be PL EY FF MB PC LP MR SK or TS if age = 2");
            }
            // if sex = U hs can be blank
            if (!(hs.equals("PL") || hs.equals("WL") || hs.equals(""))) {
                line.setCustomNotes(  "     HS/AGE MISMATCH***: how sexed must be PL or WL if age = 2");
            }
        } else if (age == 4) {
            ...
            if (!(ha.equals("MR") || ha.equals("NF") && line.getFFMlt().equals("J"))) {
                line.setCustomNotes(
                    "     HA/AGE MISMATCH***: how aged must be MR and ff molt must be J if age = 4");
            }
            if (!(ha2.equals("PL") || ha2.equals("EY") || ha2.equals("FF") || ha2.equals("MB") || ha2.equals("PC")
                || ha2.equals("LP") || ha2.equals("SK") || ha2.equals("PC") || ha2.equals("TS")
                || ha2.equals(""))) {
                line.setCustomNotes(
                    "     HA2/AGE MISMATCH***: how aged 2 must be PL EY FF MB PC LP SK TS or blank if age = 2");
            }
        }
    ...
    public void proofParasites(DataLine line) {
        String parasites = line.hasParasites();
        String notes = line.getNotes();
        if(!(parasites.equals("Y") || parasites.equals("N"))) {
            line.setCustomNotes("       PARASITES***: Parasites must either be Y or N");
        }
        if (parasites.equals("Y") && notes.equals("")) {
            line.setCustomNotes(
                "     PARASITES***: If there is a Y in the parasites column there needs to be a note");
        }
    }
    ...
    public void proofhaha2(DataLine line) {
        ...
        if (!(ha.equals("") || ha2.equals(""))) {
            if (ha.equals(ha2)) {
                line.setCustomNotes("     HA/HA2***: if both how aged columns are filled both cannot be the same");
            }
        }
    }
    ...
    public void proofhshs2(DataLine line) {
        ...
        if (!(hs.equals("") || hs2.equals(""))) {
            if (hs.equals(hs2)) {
                line.setCustomNotes("     HS/HS2***: if both how sexed columns are filled both cannot be the same");
            }
        }
    }
    
    public void proofSite(DataLine line) {
        String site = line.getSite();
        if(site.equals("")) {
            line.setCustomNotes("       SITE***: Site cannot be blank");
        }
    }
}